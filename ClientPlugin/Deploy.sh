...
# Copies the built plugin DLL into the Pulsar "Local" plugin folder.
# On Linux Pulsar stores its data under ~/.config/Pulsar (no "Legacy" subfolder).
...
PULSAR_DIR="$HOME/.config/Pulsar"
if [ ! -d "$PULSAR_DIR" ]; then
    echo "Missing Pulsar folder: $PULSAR_DIR" >&2
    echo "Pulsar not installed?" >&2
    exit 2
fi

PLUGIN_DIR="$PULSAR_DIR/Local"
mkdir -p "$PLUGIN_DIR"
